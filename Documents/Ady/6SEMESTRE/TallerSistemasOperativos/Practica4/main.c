...
void encabezado()
{
    gotoxy(50,3);printf("----- PRACTICA NO.4 -----");
...
void Proceso(COLAPROCESO *NUEVOS,int cantidadProcesos,COLAPROCESO *LISTOS,COLAPROCESO *CPN,COLAPROCESOTERMINADO *CPT)
{
    int relojGlobal=0;
    int contAux=0;
    int contMemoria=0;
    ...
    while(aux != NULL)
    {
        ...
        else
        {
            if(bndErrorProceso == 1)
            {
                ...
                proceso.tFinalizacion=relojGlobal-contProceso;
                proceso.tRetorno=proceso.tFinalizacion-proceso.tLlegada;
                proceso.tEspera=proceso.tRetorno-procesoTerminado.auxTiempo;
...
int main()
{
    ...
    Proceso(&NUEVOS,cantidadProcesos,&LISTOS,&CPN,&CPT);
    getch();
    return 0;
}
