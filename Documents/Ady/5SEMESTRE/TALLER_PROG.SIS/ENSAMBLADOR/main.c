...
COD_MAQ RecuperaCodigoMaquina(FILE * fdTabOp,char codop[12],char operando[10])
{
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                {
                                                    COD_MAQUINA.REL=1;
                                                    strcpy(COD_MAQUINA.codmaqREL,TABOP.codMaq);
                                                    COD_MAQUINA.sumaBytesREL=TABOP.bytes_suma;
                                                }
                                            }
                                        }
                                    }
    ...
    if((COD_MAQUINA.REL == 1) && (bandAuxRel == 1))
    {
        if(COD_MAQUINA.sumaBytesREL == '4')
            COD_MAQUINA.bandera=17; //Relativo 16 bits
        else
        {
            if(COD_MAQUINA.sumaBytesREL == '2')
                COD_MAQUINA.bandera=16;  //Relativo 8 bits
        }
    }
...
int localizaTabsim(FILE *fdTabsim,char operando[10],char dirTemp[5],int tipoRel)
{
    char cadena[15];
    char etiqueta[10];
    char direccion[5];
    ...
    fseek(fdTabsim,0,0);
    while(fgets(cadena,15,fdTabsim))
    {
        for(i=0; i<10; i++)
            etiqueta[i]='\0';
        for(i=0; i<5; i++)
        ...
        if(aux == 0)
        {
            ...
            valorDecimalDireccion=numero_total;
            ...
            sigValor=numero_total;
            if(tipoRel == 1)
                sigValor=numero_total += 2;
            else
                sigValor=numero_total += 4;
            desplazamiento=valorDecimalDireccion-sigValor;
        }
    }
    return desplazamiento;
}

void recuperaRuta(char* cadRegistro)
{
    getcwd(cadRegistro,_MAX_PATH);
}
...
int main()
{
    ...
    char cadLongitud[3];
    char cadChecksum[50];
    char cadenaTemporal[50];
    int contAux=0;
    int contAux1=0;
    int numeroChecksum=0;
    char cadAuxRegistro[50];
    char cadenaAux[130];
    char cadRegistro[_MAX_PATH];
    int bandTemporal=0;
    int bandAuxReg=0;
    int bandArchObjeto=0;
    char cadComp[20];
    int bandTabsim=0;
    char nombreRegistro[15];
    ...
    char et[10];
    char direccion[5];
    char cadenaTabsim[15];
    char car;
    char cadConstante[20];
    ...
    for(i=0; i<3; i++)
        cadLongitud[i]='\0';
    for(i=0; i<3; i++)
        REGISTRO.checksum[i]='\0';
    for(i=0; i<5; i++)
        REGISTRO.direccion[i]='\0';
    for(i=0; i<50; i++)
        cadAuxRegistro[i]='\0';
    for(i=0; i<50; i++)
        REGISTROS1.RegS1[i]='\0';
    for(i=0; i<5; i++)
        REGISTROS1.dirS1[i]='\0';
    for(i=0; i<3; i++)
        REGISTROS1.checksumS1[i]='\0';
    for(i=0; i<20; i++)
        cadComp[i]='\0';
    for(i=0; i<20; i++)
        codMaqTemp[i]='\0';
    for(i=0; i<10; i++)
        REGISTROS1.longFinal[i]='\0';
    REGISTRO.longitud=0;

    recuperaRuta(cadRegistro);

    printf("\n\n\t\tIngresa el Nombre del Archivo: ");
    gets(nombreRegistro);

    strcat(nombreRegistro,".txt");

    fd=fopen(nombreRegistro,"r");
    if(fd != NULL)
    {
        ...
        fdAuxReg=fopen("AUXILIAR_REGISTRO.txt","w+");
        if(fdAuxReg != NULL)
            bandAuxReg=1;

        fdarchObjeto=fopen("ArchObjeto.txt","w+");
        if(fdarchObjeto != NULL)
            bandArchObjeto=1;
        ...
        if(bandTemporal == 1)
        {
            strcat(nombreCompleto,cadRegistro);
            strcat(nombreCompleto,"\\");
            strcat(nombreCompleto,nombreRegistro);
            strcat(nombreCompleto,"\n");

            for(i=0; i<strlen(nombreCompleto); i++)
            {
                caracterTemporal=nombreCompleto[i];
                numeroAuxiliar=caracterTemporal;
                numeroChecksum=numeroAuxiliar;
                itoa(numeroAuxiliar,cadNumero,16);
                strupr(cadNumero);
                if(strlen(cadNumero) < 2)
                {
                    aux=0;
                    longitud=strlen(cadNumero);
                    total=2-longitud;
                    for(j=0; j<total; j++)
                    {
                        cadenaAuxiliar[j]='0';
                    ...
                    for(j=aux; j<2; j++)
                    {
                        cadenaAuxiliar[j]=cadNumero[x];
                        x++;
                    }
                    strcpy(cadNumero,cadenaAuxiliar);
                }
                strcat(cadenaAux,cadNumero);
                sumaChecksum+=numeroChecksum;
            }
            itoa((~sumaChecksum)+1,cadComplemento,16);
            REGISTRO.checksum[0]=cadComplemento[strlen(cadComplemento)-2];
            REGISTRO.checksum[1]=cadComplemento[strlen(cadComplemento)-1];
            strupr(REGISTRO.checksum);

            strcpy(REGISTRO.S0,"S0");
            strcpy(REGISTRO.direccion,"0000");
            longitudTemporal=(strlen(cadenaAux) / 2) +3;
            itoa(longitudTemporal,cadLongitud,16);
            strcat(REGISTRO.S0,cadLongitud);
            strcat(REGISTRO.S0,REGISTRO.direccion);
            strcat(REGISTRO.S0,cadenaAux);
            strcat(REGISTRO.S0,REGISTRO.checksum);
            printf("Registro S0: %s",REGISTRO.S0);
            if(bandArchObjeto == 1)
            {
                fseek(fdarchObjeto,0,2);
                fprintf(fdarchObjeto,"%s\n",REGISTRO.S0);
            }
            ...
            while(fgets(cadenaTemporal,30,fdTemp))
            {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        if((operandoTemp[0] >= 65) || (operandoTemp[0] >= 97))
                                        {
                                            cont=0;
                                            j=0;
                                            fseek(fdTabsim,0,0);
                                            while(fgets(cadenaTabsim,15,fdTabsim))
                                            {
                                                for(i=0; i<10; i++)
                                                    et[i]='\0';
                                                for(i=0; i<5; i++)
                                                    direccion[i]='\0';
                                                for(i=0; i<strlen(cadenaTabsim); i++)
                                                {
                                                    caracter=cadenaTabsim[i];
                                                    if((caracter != '\t') && (caracter != '\n'))
                                                    {
                                                        switch(cont)
                                                        {
                                                ...
                                                cont=0;
                                                j=0;
                                                aux=strcmp(operandoTemp,et);
                                                if(aux == 0)
                                                    strcat(CODIGO_MAQUINA.codmaqEXT,direccion);
                                            }
                                        }
                                    }
                                }
                                                    ...
                                                    else
                                                    {
                                                        if(CODIGO_MAQUINA.bandera == 14)
                                                        {
                                                            ...
                                                            for(i=0; i<strlen(cadAux); i++)
                                                            {
                                                                ...
                                                                if((bandComa == 0) && (cadAux[i] != '[') && (cadAux[i] != ']'))
                                                                {
                                                                    cad_numero[m]=cadAux[i];
                                                                    m++;
                                                                }
                                                                else
                                                                {
                                                                    if((cadAux[i] != ',') && (cadAux[i] != '[') && (cadAux[i] != ']'))
                                                                    {
                                                                        registro[n]=cadAux[i];
                                                                        n++;
                                                                    }
                                                            ...
                                                            XB=RecuperaXB(operandoAuxiliar,"16BINDR");
                                                            strcat(CODIGO_MAQUINA.codmaq_IDX2,XB.xb);

                                                            valorDecimal=numeroAux;
                                                            itoa(valorDecimal,op,16);
                                                            strupr(op);
                                                            if(strlen(op) == 4)
                                                                strcat(CODIGO_MAQUINA.codmaq_IDX2,op);
                                                            else
                                                            {
                                                                aux=0;
                                                                longitud=strlen(op);
                                                                ...
                                                                strcat(CODIGO_MAQUINA.codmaq_IDX2,cadenaAuxiliar);
                                                            }
                                                            printf("\n\n\t%s\t%s\t%s\t%s\t\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp,CODIGO_MAQUINA.codmaq_IDX2);
                                                            if(bandAuxReg == 1)
                                                            {
                                                                fseek(fdAuxReg,0,2);
                                                                fprintf(fdAuxReg,"%s\t%s\t%s\t%s\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp,CODIGO_MAQUINA.codmaq_IDX2);
                                                        ...
                                                        {
                                                            if(CODIGO_MAQUINA.bandera == 15)
                                                            {
                                                                XB=RecuperaXB(operandoTemp,"DINDIRECT");
                                                                strcat(CODIGO_MAQUINA.codmaqDIDX,XB.xb);
                                                                printf("\n\n\t%s\t%s\t%s\t%s\t\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp,CODIGO_MAQUINA.codmaqDIDX);
                                                                if(bandAuxReg == 1)
                                                                {
                                                                    fseek(fdAuxReg,0,2);
                                                                    fprintf(fdAuxReg,"%s\t%s\t%s\t%s\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp,CODIGO_MAQUINA.codmaqDIDX);
                                                            ...
                                                            {
                                                                if(CODIGO_MAQUINA.bandera == 16)
                                                                {
                                                                    desplazamiento=localizaTabsim(fdTabsim,operandoTemp,valorTemp,1);
                                                                    if((desplazamiento > -127) && (desplazamiento < 128))
                                                                    {
                                                                        if(desplazamiento >= 0)
                                                                        {
                                                                            itoa(desplazamiento,op,16);
                                                                            strupr(op);
                                                                            if(strlen(op) == 2)
                                                                                strcat(CODIGO_MAQUINA.codmaqREL,op);
                                                                            else
                                                                            {
                                                                                aux=0;
                                                                                longitud=strlen(op);
                                                                                ...
                                                                                strcat(CODIGO_MAQUINA.codmaqREL,cadenaAuxiliar);
                                                                            }
                                                                        }
                                                                        else
                                                                        {
                                                                            ...
                                                                            {
                                                                                desplazamiento *= -1;
                                                                                itoa((~desplazamiento)+1,cadComplemento,16);
                                                                                op[0]=cadComplemento[strlen(cadComplemento)-2];
                                                                                op[1]=cadComplemento[strlen(cadComplemento)-1];
                                                                                strupr(op);
                                                                                strcat(CODIGO_MAQUINA.codmaqREL,op);
                                                                        ...
                                                                        printf("\n\n\t%s\t%s\t%s\t%s\t\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp,CODIGO_MAQUINA.codmaqREL);
                                                                        if(bandAuxReg == 1)
                                                                        {
                                                                            fseek(fdAuxReg,0,2);
                                                                            fprintf(fdAuxReg,"%s\t%s\t%s\t%s\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp,CODIGO_MAQUINA.codmaqREL);
                                                                        ...
                                                                        printf("\n\n\tError el Desplazamiento Esta Fuera del Rango -127 a 128 \n");
                                                                }
                                                                else
                                                                {
                                                                    if(CODIGO_MAQUINA.bandera == 17)
                                                                    {
                                                                        desplazamiento=localizaTabsim(fdTabsim,operandoTemp,valorTemp,2);
                                                                        if((desplazamiento > -32768) && (desplazamiento < 32767))
                                                                        {
                                                                            ...
                                                                            else
                                                                            {
                                                                                if(desplazamiento < 0)
                                                                                {
                                                                                    desplazamiento *= -1;
                                                                                    itoa((~desplazamiento)+1,cadComplemento,16);
                                                                                    op[0]=cadComplemento[strlen(cadComplemento)-4];
                                                                                    op[1]=cadComplemento[strlen(cadComplemento)-3];
                                                                                    op[2]=cadComplemento[strlen(cadComplemento)-2];
                                                                                    op[3]=cadComplemento[strlen(cadComplemento)-1];
                                                                                    strupr(op);
                                                                                    strcat(CODIGO_MAQUINA.codmaqREL,op);
                                                                                }
                                                                            }
                ...
                {
                    if((strcmp(codopTemp,"DB") == 0) || (strcmp(codopTemp,"DC.B") == 0) || (strcmp(codopTemp,"FCB") == 0))
                    {
                        if(operandoTemp[0] == '$')
                        {
                            j=1;
                            for(i=0; i<strlen(operandoTemp); i++)
                            {
                                cadConstante[i]=operandoTemp[j];
                                j++;
                            }

                            if(strlen(cadConstante) < 2)
                            {
                                aux=0;
                                longitud=strlen(cadConstante);
                                total=2-longitud;
                                for(i=0; i<total; i++)
                                {
                                    cadenaAuxiliar[i]='0';
                                ...
                                for(i=aux; i<2; i++)
                                {
                                    cadenaAuxiliar[i]=cadConstante[x];
                                    x++;
                                }
                                printf("\n\n\t%s\t%s\t%s\t%s\t\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp,cadenaAuxiliar);
                                if(bandAuxReg == 1)
                                {
                                    fseek(fdAuxReg,0,2);
                                    fprintf(fdAuxReg,"%s\t%s\t%s\t%s\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp,cadenaAuxiliar);
                            ...
                            {
                                printf("\n\n\t%s\t%s\t%s\t%s\t\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp,cadConstante);
                                if(bandAuxReg == 1)
                                {
                                    fseek(fdAuxReg,0,2);
                                    fprintf(fdAuxReg,"%s\t%s\t%s\t%s\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp,cadConstante);
                        ...
                        else
                        {
                            if(operandoTemp[0] == '@')
                            {
                                valorDecimal=convierteOctalDecimal(operandoTemp);
                                itoa(valorDecimal,op,16);
                                strupr(op);
                                if(strlen(op) < 2)
                                {
                                ...
                                {
                                    printf("\n\n\t%s\t%s\t%s\t%s\t\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp,op);
                                    if(bandAuxReg == 1)
                                    {
                                        fseek(fdAuxReg,0,2);
                                        fprintf(fdAuxReg,"%s\t%s\t%s\t%s\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp,op);
                            ...
                            else
                            {
                                if(operandoTemp[0] == '%')
                                {
                                    valorDecimal=convierteBinarioDecimal(operandoTemp);
                                    itoa(valorDecimal,op,16);
                                    strupr(op);
                                    if(strlen(op) < 2)
                                    {
                    ...
                    {
                        if((strcmp(codopTemp,"DS") == 0) || (strcmp(codopTemp,"DS.B") == 0) || (strcmp(codopTemp,"RMB") == 0))
                        {
                            if(operandoTemp[0] == '$')
                            {
                                j=1;
                        ...
                        {
                            if((strcmp(codopTemp,"DS.W") == 0) || (strcmp(codopTemp,"RMW") == 0))
                            {
                                if(operandoTemp[0] == '$')
                                {
                                    j=1;
                            ...
                            {
                                if((strcmp(codopTemp,"DW") == 0) || (strcmp(codopTemp,"DC.W") == 0) || (strcmp(codopTemp,"FDB") == 0))
                                {
                                    if(operandoTemp[0] == '$')
                                    {
                                        j=1;
                                ...
                                {
                                    if(strcmp(codopTemp,"FCC") == 0)
                                    {
                                        printf("\n\n\t%s\t%s\t%s\t%s\t",valorTemp,etiquetaTemp,codopTemp,operandoTemp);
                                        if(bandAuxReg == 1)
                                        {
                                            fseek(fdAuxReg,0,2);
                                            fprintf(fdAuxReg,"%s\t%s\t%s\t%s\t",valorTemp,etiquetaTemp,codopTemp,operandoTemp);
                                        }
                                        j=1;
                                        for(i=0; i<strlen(operandoTemp); i++)
                                        {
                                    ...
                                    {
                                        if(strcmp(codopTemp,"ORG") == 0)
                                        {
                                            strcpy(valorTemp,"0000");
                                            printf("\n\n\t%s\t%s\t%s\t%s\t\tNo Encontrado\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp);
                                        }
                                        else
                                        {
                                            if(strcmp(codopTemp,"EQU") == 0)
                                            {
                                                printf("\n\n\t%s\t%s\t%s\t%s\n",valorTemp,etiquetaTemp,codopTemp,operandoTemp);
                                                if(bandAuxReg == 1)
                                                {
            ...
            for(i=0; i<20; i++)
                cadAuxRegistro[i]='\0';
            fseek(fdAuxReg,0,0);
            while(fgets(cadenaTemporal,35,fdAuxReg))
            {
                contAux++;
            }
            for(i=0; i<40; i++)
            ...
            j=0;
            fseek(fdAuxReg,0,0);
            while(fgets(cadenaTemporal,40,fdAuxReg))
            {
                contAux1++;
                for(i=0; i<5; i++)
                    valorTemp[i]='\0';
                ...
                if((strcmp(codopTemp,"DS") == 0) || (strcmp(codopTemp,"DS.B") == 0) || (strcmp(codopTemp,"DS.W") == 0) || (strcmp(codopTemp,"RMB") == 0) || (strcmp(codopTemp,"RMW") == 0))
                    bandDirectivaEspacio=1;
                if(strcmp(codopTemp,"RMB") == 0)
                    band1=1;
                difLongitud=16-REGISTROS1.longS1;
                longAuxiliar=strlen(codMaqTemp)/2;
                if(difLongitud >= longAuxiliar)
                {
                    if(REGISTROS1.longS1 == 0)
                        strcpy(REGISTROS1.dirS1,valorTemp);
                    if(bandDirectivaEspacio == 0)
                    {
                        j=strlen(cadAuxRegistro);
                        for(i=0; i<strlen(codMaqTemp); i++)
                        {
                            cadAuxRegistro[j]=codMaqTemp[i];
                            j++;
                        }
                        longTemp=strlen(codMaqTemp) / 2;
                        REGISTROS1.longS1 += longTemp;
                    }
                    if(strlen(codMaqTemp) > 0)
                        bandera1=1;
                    if(((bandDirectivaEspacio == 1) && (band1 == 0) && (bandera1 == 1)) || ((contAux == contAux1)  && (bandera1 == 1)))
                    {
                        REGISTROS1.longS1 += 3;
                        strcpy(REGISTROS1.RegS1,"S1");
                        itoa(REGISTROS1.longS1,REGISTROS1.longFinal,16);
                        strupr(REGISTROS1.longFinal);
                        if(strlen(REGISTROS1.longFinal) < 2)
                        {
                            aux=0;
                            longitud=strlen(REGISTROS1.longFinal);
                            total=2-longitud;
                            for(i=0; i<total; i++)
                            {
                                cadenaAuxiliar[i]='0';
                            ...
                            for(i=aux; i<2; i++)
                            {
                                cadenaAuxiliar[i]=REGISTROS1.longFinal[x];
                                x++;
                            }
                            strcpy(REGISTROS1.longFinal,cadenaAuxiliar);
                        }
                        strcat(REGISTROS1.RegS1,REGISTROS1.longFinal);
                        strcat(REGISTROS1.RegS1,REGISTROS1.dirS1);
                        strcat(REGISTROS1.RegS1,cadAuxRegistro);
                        strcpy(cadChecksum,REGISTROS1.longFinal);
                        strcat(cadChecksum,REGISTROS1.dirS1);
                        strcat(cadChecksum,cadAuxRegistro);
                        cont=1;
                        j=0;
                        for(i=0; i<=strlen(cadChecksum); i++)
                        {
                            if(cont <= 2)
                            {
                               caracter=cadChecksum[i];
                                CadAux[j]=caracter;
                                j++;
                                cont++;
                            }
                            ...
                            {
                                numeroAux=ConvHexDec(CadAux);
                                sumChecksum += numeroAux;
                                i--;
                                cont=1;
                                j=0;
                            }
                        ...
                        for(i=0; i<10; i++)
                            cadAuxRegistro[i]='\0';
                        itoa((~sumChecksum),cadAuxRegistro,16);
                        REGISTROS1.checksumS1[0]=cadAuxRegistro[strlen(cadAuxRegistro)-2];
                        REGISTROS1.checksumS1[1]=cadAuxRegistro[strlen(cadAuxRegistro)-1];
                        REGISTROS1.checksumS1[2]='\0';
                        strupr(REGISTROS1.checksumS1);
                        strcat(REGISTROS1.RegS1,REGISTROS1.checksumS1);
                        printf("\n\n\n\t\tRegistro S1: %s",REGISTROS1.RegS1);
                        if(bandArchObjeto == 1)
                        {
                            fseek(fdarchObjeto,0,2);
                            fprintf(fdarchObjeto,"%s\n",REGISTROS1.RegS1);
                ...
                {
                    Aux=difLongitud - longAuxiliar;
                    if(Aux < 0)
                    {
                        Aux=longAuxiliar-difLongitud;
                        if(Aux == longAuxiliar)
                        {
                            REGISTROS1.longS1 += 3;
                            strcpy(REGISTROS1.RegS1,"S1");
                        ...
                        {
                            n=0;
                            for(k=0; k<(difLongitud*2); k++)
                            {
                                cadReg[k]=codMaqTemp[k];
                            }
                            aux=strlen(codMaqTemp)-(difLongitud*2);
                            for(m=0; m<aux ; m++)
                            {
                                cadReg1[m]=codMaqTemp[k];
                                k++;
                            }
                            j=strlen(cadAuxRegistro);
                            for(i=0; i<strlen(codMaqTemp); i++)
                            {
                                cadAuxRegistro[j]=cadReg[i];
                                j++;
                            }
                            longTemp=strlen(cadReg) / 2;
                            REGISTROS1.longS1 += longTemp;
                            REGISTROS1.longS1 += 3;
                            strcpy(REGISTROS1.RegS1,"S1");
                            itoa(REGISTROS1.longS1,REGISTROS1.longFinal,16);
                            ...
                            strcat(cadChecksum,REGISTROS1.dirS1);
                            strcat(cadChecksum,cadAuxRegistro);
                            numeroAux=ConvHexDec(REGISTROS1.dirS1);
                            valDireccion=numeroAux+(strlen(cadAuxRegistro)/2);
                            itoa(valDireccion,REGISTROS1.dirS1,16);
                            cont=1;
                            j=0;
                            for(i=0; i<=strlen(cadChecksum); i++)
                            {
                            ...
                            sumChecksum=0;
                            j=strlen(cadAuxRegistro);
                            for(i=0; i<strlen(cadReg1); i++)
                            {
                                cadAuxRegistro[j]=cadReg1[i];
                                j++;
                            }
                            longTemp=strlen(cadReg1) / 2;
                            REGISTROS1.longS1 += longTemp;
                        }
        ...
        strcpy(REGISTRO.S9,"S9030000FC");
        printf("\n\n\n\t\tRegistro S9: %s",REGISTRO.S9);
        if(bandArchObjeto == 1)
        {
            fseek(fdarchObjeto,0,2);
            fprintf(fdarchObjeto,"%s\n",REGISTRO.S9);
