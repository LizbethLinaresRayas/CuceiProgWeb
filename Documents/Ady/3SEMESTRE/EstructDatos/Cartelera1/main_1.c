...
void baja_peli(pelicula *mov,hora_sala *hor)
{
    int i,x;
...
void ordena(pelicula *mov)
{
    ...
    scanf("%d",&x);
    if(x==1)
        ordena_insercion(pelicula x,int n);
    else
        ordena_quicksore(pelicula x,int primero,int ultimo);
}

int ordena_insercion(pelicula *mov,int n)
{
